...
private:
   ...
   int nTprime = 0;
   int nHtHt = 0, nWbWb = 0, nZtZt = 0, nWbHt = 0, nWbZt = 0, nHtZt = 0;
...
void skeletonAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   int nSuu_W = 0, nSuu_b = 0, nW = 0, nTop = 0, nZ = 0, nH = 0;
   ...
   for (auto iG = genParticles->begin(); iG != genParticles->end(); iG++) 
   {
      ...
      if ((abs(iG->pdgId()) == 24) && (abs(iG->mother()->pdgId()) == Tprime_pdgid)   ) nSuu_W++;
      if ((abs(iG->pdgId()) == 5) && (abs(iG->mother()->pdgId()) == Tprime_pdgid)   ) nSuu_b++;
      if ((abs(iG->pdgId()) == 24) && (abs(iG->mother()->pdgId()) == 6)   ) nW++;
      if ((abs(iG->pdgId()) == 23) && (abs(iG->mother()->pdgId()) == Tprime_pdgid)   ) nZ++;
      if ((abs(iG->pdgId()) == 6) && (abs(iG->mother()->pdgId()) == Tprime_pdgid)   ) nTop++;
      if ((abs(iG->pdgId()) == 25) && (abs(iG->mother()->pdgId()) == Tprime_pdgid)   ) nH++;


   }

   if (( nSuu_W == 2) && (nSuu_b == 2)) nWbWb++;
   else if (( nTop == 2) && (nZ == 2)) nZtZt++;
   else if (( nTop == 2) && (nH == 2)) nHtHt++;
   else if (( nSuu_W == 1) && (nSuu_b == 1) && (nTop== 1) && (nZ ==1 )) nWbZt++;
   else if (( nSuu_W == 1) && (nSuu_b == 1) && (nTop ==1)  && (nH == 1)) nWbHt++;
   else if (( nZ == 1) && (nTop== 2) && (nH == 1)) nHtZt++;

   //std::cout << "HtHt/ZtZt/WbWb/HtZt/WbHt/WbZt = " << nHtHt << "/" << nZtZt << "/" <<  nWbWb << "/" << nHtZt  << "/" << nWbHt  << "/" << nWbZt << std::endl;
   //std::cout << "nSuu_W/nSuu_b/nW/nTop/nZ/nH: " <<nSuu_W << "/" <<nSuu_b<< "/" << nW<< "/" << nTop<< "/" << nZ<< "/" << nH<< std::endl;
   ///////////////////////////AK4 Jets////////////////////////////////
   ...
   for(auto iJet = fatJets->begin(); iJet != fatJets->end(); iJet++)         ////////Over AK8 Jets
   {
      ...
      // limit to jet masses consistent with top quarks and pt between 900 GeV and 1200 GeV
      if ( (iJet->mass() > 190)  || (iJet->mass() < 160) ) continue;
      if ((iJet->pt() > 1200) || (iJet->pt() < 900)) continue;

      // check to see if there is a top quark nearby && check if the jet pt is 900 GeV -> 1200 GeV && check if the mass if 160 GeV -> 190 GeV
      for (const auto& genParticle : *genParticles) 
      {
      if ( ( abs(genParticle.pdgId()) == 6)  && (genParticle.isLastCopy()) )
      {
          double deltaR = reco::deltaR(genParticle.eta(), genParticle.phi(), iJet->eta(), iJet->phi());
         if (deltaR < 0.8) 
          {
              // all conditions are met, print out the jet mass, pt, 3-vector and daughter 3-vectors

            std::cout << "----------------------------- Top quark found ----------------------------- " << std::endl;
            std::cout << " top quark mass / pt / 3-vector: " << iJet->mass() << " / " << iJet->pt() << " / (" << iJet->px() << ", " << iJet->py() << ", " << iJet->pz() << ")" << std::endl;
            std::cout << "daughter particle information --- " << std::endl;
            for(unsigned int iii = 0; iii < iJet->numberOfDaughters(); iii++)
            {
               const reco::Candidate* iDaughter = iJet->daughter(iii);
               std::cout << "daughter #" << iii << " --- (" << iDaughter->px() << ", " << iDaughter->py() << ", " << iDaughter->pz() << ")" << std::endl;
            }
